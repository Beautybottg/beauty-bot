#!/usr/bin/env python3
# -*- coding: utf-8 -*-

import os
import logging
import json
from datetime import datetime, timedelta
from typing import Dict, List, Optional, Tuple

from telegram import (
    Update, 
    InlineKeyboardButton, 
    InlineKeyboardMarkup,
    ReplyKeyboardMarkup,
    KeyboardButton
)
from telegram.ext import (
    Application, 
    CommandHandler, 
    CallbackQueryHandler, 
    MessageHandler, 
    ContextTypes,
    ConversationHandler,
    filters
)
from telegram.constants import ParseMode

# ========== НАСТРОЙКИ ==========
import os
TOKEN = os.environ.get("8058974826:AAHy1iYv34O3xhB_vj6h6a0Ryme2a0OKEio")  # Замените на ваш токен!
ADMIN_IDS = [651764371]  # Замените на ваш ID администратора (узнать через @userinfobot)

# Настройка логирования
logging.basicConfig(
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    level=logging.INFO
)
logger = logging.getLogger(__name__)

# Состояния для ConversationHandler
SELECT_SERVICE, SELECT_DATE, SELECT_TIME, ENTER_NAME, ENTER_PHONE, ENTER_COMMENT, CONFIRMATION = range(7)
CANCEL_APPOINTMENT = 8

# ========== БАЗА ДАННЫХ (простая JSON) ==========
DB_FILE = "appointments.json"

def load_appointments() -> Dict:
    """Загрузка записей из файла"""
    try:
        with open(DB_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        return {"appointments": {}, "counters": {"next_id": 1}}

def save_appointments(data: Dict):
    """Сохранение записей в файл"""
    with open(DB_FILE, 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

def add_appointment(appointment: Dict) -> int:
    """Добавление новой записи"""
    data = load_appointments()
    appointment_id = data["counters"]["next_id"]
    
    # Добавляем дату создания
    appointment["created_at"] = datetime.now().isoformat()
    appointment["status"] = "active"  # active, cancelled, completed
    
    data["appointments"][str(appointment_id)] = appointment
    data["counters"]["next_id"] += 1
    
    save_appointments(data)
    return appointment_id

def get_user_appointments(user_id: int) -> List[Tuple[int, Dict]]:
    """Получение всех записей пользователя"""
    data = load_appointments()
    user_appointments = []
    
    for app_id, appointment in data["appointments"].items():
        if str(user_id) in appointment.get("user_id", ""):
            user_appointments.append((int(app_id), appointment))
    
    # Сортируем по дате
    user_appointments.sort(key=lambda x: x[1].get("date", ""))
    return user_appointments

def cancel_appointment(appointment_id: int, cancelled_by: str = "client") -> bool:
    """Отмена записи"""
    data = load_appointments()
    
    if str(appointment_id) in data["appointments"]:
        appointment = data["appointments"][str(appointment_id)]
        appointment["status"] = "cancelled"
        appointment["cancelled_at"] = datetime.now().isoformat()
        appointment["cancelled_by"] = cancelled_by
        save_appointments(data)
        return True
    
    return False

def get_appointment(appointment_id: int) -> Optional[Dict]:
    """Получение записи по ID"""
    data = load_appointments()
    return data["appointments"].get(str(appointment_id))

# ========== УСЛУГИ И РАСПИСАНИЕ ==========
SERVICES = {
    "manicure": {
        "name": "💅 Маникюр",
        "price": "1500-2500 руб",
        "duration": "60-90 мин",
        "description": "Классический, аппаратный, покрытие гель-лаком"
    },
    "pedicure": {
        "name": "🦶 Педикюр",
        "price": "2000-3000 руб",
        "duration": "90-120 мин",
        "description": "Аппаратный, европейский, мужской"
    },
    "eyelash": {
        "name": "👁️ Наращивание ресниц",
        "price": "2500-4000 руб",
        "duration": "120-180 мин",
        "description": "Классика, 2D, 3D, объемное наращивание"
    },
    "brows": {
        "name": "✏️ Оформление бровей",
        "price": "800-1500 руб",
        "duration": "30-60 мин",
        "description": "Коррекция, окрашивание, ламинирование"
    },
    "facial": {
        "name": "🌸 Чистка лица",
        "price": "1800-3500 руб",
        "duration": "60-90 мин",
        "description": "Механическая, ультразвуковая, атравматичная"
    },
    "haircut": {
        "name": "💇 Стрижка",
        "price": "1200-2500 руб",
        "duration": "60 мин",
        "description": "Женская, мужская, детская стрижка"
    },
    "makeup": {
        "name": "💄 Макияж",
        "price": "2000-4000 руб",
        "duration": "60-90 мин",
        "description": "Дневной, вечерний, свадебный"
    }
}

# Доступное время
TIME_SLOTS = [
    "09:00", "10:30", "12:00", "13:30", 
    "15:00", "16:30", "18:00", "19:30"
]

# ========== КОМАНДЫ БОТА ==========
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /start"""
    user = update.effective_user
    
    welcome_text = f"""
✨ *Добро пожаловать, {user.first_name}!* ✨

Я — бот салона красоты *Beauty Elegance* 🏩

*Что я умею:*
✅ Записать на услугу
📋 Показать мои записи
❌ Отменить запись
📞 Связаться с администратором

*Выберите действие:*"""

    keyboard = [
        [InlineKeyboardButton("📅 Записаться на услугу", callback_data="new_appointment")],
        [InlineKeyboardButton("📋 Мои записи", callback_data="my_appointments")],
        [InlineKeyboardButton("❌ Отменить запись", callback_data="cancel_appointment")],
        [InlineKeyboardButton("📞 Контакты", callback_data="contacts")],
        [InlineKeyboardButton("ℹ️ О салоне", callback_data="about")]
    ]
    
    if user.id in ADMIN_IDS:
        keyboard.append([InlineKeyboardButton("👑 Админ-панель", callback_data="admin_panel")])
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await update.message.reply_text(
        welcome_text, 
        reply_markup=reply_markup,
        parse_mode=ParseMode.MARKDOWN
    )
    
    return ConversationHandler.END

async def new_appointment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Начало записи на услугу"""
    query = update.callback_query
    await query.answer()
    
    # Сбрасываем данные пользователя
    context.user_data.clear()
    
    # Показываем выбор услуги
    keyboard = []
    row = []
    for service_id, service in SERVICES.items():
        button = InlineKeyboardButton(
            service["name"], 
            callback_data=f"service_{service_id}"
        )
        row.append(button)
        if len(row) == 2:
            keyboard.append(row)
            row = []
    if row:
        keyboard.append(row)
    
    keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="back_to_menu")])
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(
        "🎨 *Выберите услугу:*\n\n" +
        "\n".join([f"• {s['name']} ({s['price']})" for s in SERVICES.values()]),
        reply_markup=reply_markup,
        parse_mode=ParseMode.MARKDOWN
    )
    
    return SELECT_SERVICE

async def select_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выбор услуги"""
    query = update.callback_query
    await query.answer()
    
    service_id = query.data.replace("service_", "")
    service = SERVICES[service_id]
    
    # Сохраняем выбранную услугу
    context.user_data["service"] = service
    context.user_data["service_id"] = service_id
    
    # Показываем выбор даты
    keyboard = []
    today = datetime.now()
    
    # Создаем кнопки на 14 дней вперед
    for i in range(14):
        date = today + timedelta(days=i)
        date_str = date.strftime("%d.%m.%Y")
        weekday = date.strftime("%a")
        
        button = InlineKeyboardButton(
            f"{date_str} ({weekday})", 
            callback_data=f"date_{date.strftime('%Y-%m-%d')}"
        )
        keyboard.append([button])
    
    keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="back_to_services")])
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(
        f"📅 *Выбрано:* {service['name']}\n\n"
        f"*Цена:* {service['price']}\n"
        f"*Время:* {service['duration']}\n"
        f"*Описание:* {service['description']}\n\n"
        "*Выберите дату:*",
        reply_markup=reply_markup,
        parse_mode=ParseMode.MARKDOWN
    )
    
    return SELECT_DATE

async def select_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выбор даты"""
    query = update.callback_query
    await query.answer()
    
    date_str = query.data.replace("date_", "")
    context.user_data["date"] = date_str
    
    # Показываем выбор времени
    keyboard = []
    row = []
    for time in TIME_SLOTS:
        button = InlineKeyboardButton(time, callback_data=f"time_{time}")
        row.append(button)
        if len(row) == 4:
            keyboard.append(row)
            row = []
    if row:
        keyboard.append(row)
    
    keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="back_to_services")])
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    # Форматируем дату
    date_obj = datetime.strptime(date_str, "%Y-%m-%d")
    formatted_date = date_obj.strftime("%d.%m.%Y (%A)")
    
    await query.edit_message_text(
        f"📅 *Дата:* {formatted_date}\n\n"
        "*Выберите время:*",
        reply_markup=reply_markup,
        parse_mode=ParseMode.MARKDOWN
    )
    
    return SELECT_TIME

async def select_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выбор времени"""
    query = update.callback_query
    await query.answer()
    
    time = query.data.replace("time_", "")
    context.user_data["time"] = time
    
    # Проверяем, есть ли уже имя пользователя
    user = update.effective_user
    if user.first_name:
        context.user_data["name"] = user.first_name
        if user.last_name:
            context.user_data["name"] += f" {user.last_name}"
    
    await query.edit_message_text(
        f"⏰ *Время:* {time}\n\n"
        "📝 *Введите ваше имя:*\n"
        "(Можно ввести новое или оставить текущее)",
        parse_mode=ParseMode.MARKDOWN
    )
    
    return ENTER_NAME

async def enter_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Ввод имени"""
    name = update.message.text.strip()
    context.user_data["name"] = name
    
    # Предлагаем поделиться контактом
    keyboard = [[
        KeyboardButton("📱 Поделиться контактом", request_contact=True)
    ]]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=True)
    
    await update.message.reply_text(
        f"👤 *Имя:* {name}\n\n"
        "*Введите номер телефона:*\n"
        "(Или нажмите кнопку ниже)",
        reply_markup=reply_markup,
        parse_mode=ParseMode.MARKDOWN
    )
    
    return ENTER_PHONE

async def enter_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Ввод телефона"""
    if update.message.contact:
        phone = update.message.contact.phone_number
    else:
        phone = update.message.text.strip()
    
    context.user_data["phone"] = phone
    
    # Убираем клавиатуру
    await update.message.reply_text(
        f"📱 *Телефон:* {phone}\n\n"
        "💬 *Введите комментарий к записи:*\n"
        "(Можно пропустить, отправив /skip)\n\n"
        "Пример: аллергия на цитрусовые, предпочтительный мастер и т.д.",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=ReplyKeyboardMarkup([[]], resize_keyboard=True)  # Убираем клавиатуру
    )
    
    return ENTER_COMMENT

async def enter_comment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Ввод комментария"""
    if update.message.text and update.message.text != "/skip":
        context.user_data["comment"] = update.message.text
    else:
        context.user_data["comment"] = "Без комментариев"
    
    return await show_confirmation(update, context)

async def show_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показ подтверждения записи"""
    data = context.user_data
    
    # Форматируем дату
    date_obj = datetime.strptime(data["date"], "%Y-%m-%d")
    formatted_date = date_obj.strftime("%d.%m.%Y (%A)")
    
    confirmation_text = f"""
✅ *ПОДТВЕРЖДЕНИЕ ЗАПИСИ*

*Услуга:* {data['service']['name']}
*Дата:* {formatted_date}
*Время:* {data['time']}
*Имя:* {data['name']}
*Телефон:* {data['phone']}
*Комментарий:* {data['comment']}

*Цена:* {data['service']['price']}
*Длительность:* {data['service']['duration']}

Всё верно?"""
    
    keyboard = [
        [InlineKeyboardButton("✅ Подтвердить запись", callback_data="confirm_appointment")],
        [InlineKeyboardButton("✏️ Исправить", callback_data="edit_appointment")],
        [InlineKeyboardButton("❌ Отменить", callback_data="cancel")]
    ]
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    if update.callback_query:
        await update.callback_query.edit_message_text(
            confirmation_text,
            reply_markup=reply_markup,
            parse_mode=ParseMode.MARKDOWN
        )
    else:
        await update.message.reply_text(
            confirmation_text,
            reply_markup=reply_markup,
            parse_mode=ParseMode.MARKDOWN
        )
    
    return CONFIRMATION

async def confirm_appointment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Подтверждение записи"""
    query = update.callback_query
    await query.answer()
    
    data = context.user_data
    user = update.effective_user
    
    # Создаем запись
    appointment = {
        "user_id": str(user.id),
        "user_username": user.username,
        "service": data["service"]["name"],
        "service_id": data["service_id"],
        "date": data["date"],
        "time": data["time"],
        "name": data["name"],
        "phone": data["phone"],
        "comment": data["comment"],
        "price": data["service"]["price"],
        "duration": data["service"]["duration"]
    }
    
    # Сохраняем запись
    appointment_id = add_appointment(appointment)
    
    # Форматируем дату
    date_obj = datetime.strptime(data["date"], "%Y-%m-%d")
    formatted_date = date_obj.strftime("%d.%m.%Y")
    
    # Отправляем подтверждение клиенту
    await query.edit_message_text(
        f"""
🎉 *ЗАПИСЬ ПОДТВЕРЖДЕНА!* 🎉

*Номер записи:* #{appointment_id}
*Услуга:* {data['service']['name']}
*Дата:* {formatted_date}
*Время:* {data['time']}
*Мастер:* будет назначен

*Наш адрес:* ул. Красоты, 15
*Телефон:* +7 (999) 123-45-67

⚠️ *Пожалуйста:*
• Приходите за 5-10 минут до записи
• Сообщите об отмене минимум за 3 часа
• Возьмите с собой маску""",
        parse_mode=ParseMode.MARKDOWN
    )
    
    # Отправляем уведомление администратору
    for admin_id in ADMIN_IDS:
        try:
            await context.bot.send_message(
                chat_id=admin_id,
                text=f"""
📋 *НОВАЯ ЗАПИСЬ* #{appointment_id}

*Клиент:* {data['name']}
*Телефон:* {data['phone']}
*Услуга:* {data['service']['name']}
*Дата:* {formatted_date}
*Время:* {data['time']}
*Комментарий:* {data['comment']}

*ID пользователя:* {user.id}
*Username:* @{user.username if user.username else 'нет'}""",
                parse_mode=ParseMode.MARKDOWN
            )
        except Exception as e:
            logger.error(f"Не удалось отправить уведомление админу {admin_id}: {e}")
    
    # Очищаем данные пользователя
    context.user_data.clear()
    
    return ConversationHandler.END

async def my_appointments(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показать мои записи"""
    query = update.callback_query
    await query.answer()
    
    user_id = update.effective_user.id
    appointments = get_user_appointments(user_id)
    
    if not appointments:
        await query.edit_message_text(
            "📭 У вас нет активных записей.",
            parse_mode=ParseMode.MARKDOWN
        )
        return ConversationHandler.END
    
    text = "📋 *ВАШИ ЗАПИСИ:*\n\n"
    
    for app_id, appointment in appointments:
        if appointment.get("status") != "active":
            continue
            
        date_obj = datetime.strptime(appointment["date"], "%Y-%m-%d")
        formatted_date = date_obj.strftime("%d.%m.%Y")
        
        text += f"""
*Запись #{app_id}*
{appointment['service']}
*Дата:* {formatted_date}
*Время:* {appointment['time']}
*Статус:* {appointment.get('status', 'active')}
─────────────────────
"""
    
    keyboard = [
        [InlineKeyboardButton("❌ Отменить запись", callback_data="cancel_appointment_list")],
        [InlineKeyboardButton("🔙 Главное меню", callback_data="back_to_menu")]
    ]
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(
        text,
        reply_markup=reply_markup,
        parse_mode=ParseMode.MARKDOWN
    )
    
    return ConversationHandler.END

async def cancel_appointment_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Меню отмены записи"""
    query = update.callback_query
    await query.answer()
    
    user_id = update.effective_user.id
    appointments = get_user_appointments(user_id)
    
    # Фильтруем только активные записи
    active_appointments = [
        (app_id, app) for app_id, app in appointments 
        if app.get("status") == "active"
    ]
    
    if not active_appointments:
        await query.edit_message_text(
            "📭 У вас нет активных записей для отмены.",
            parse_mode=ParseMode.MARKDOWN
        )
        return ConversationHandler.END
    
    keyboard = []
    for app_id, appointment in active_appointments:
        date_obj = datetime.strptime(appointment["date"], "%Y-%m-%d")
        formatted_date = date_obj.strftime("%d.%m")
        
        button_text = f"#{app_id} - {appointment['service']} {formatted_date} {appointment['time']}"
        callback_data = f"cancel_{app_id}"
        
        keyboard.append([InlineKeyboardButton(button_text, callback_data=callback_data)])
    
    keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="back_to_menu")])
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(
        "❌ *ВЫБЕРИТЕ ЗАПИСЬ ДЛЯ ОТМЕНЫ:*",
        reply_markup=reply_markup,
        parse_mode=ParseMode.MARKDOWN
    )
    
    return CANCEL_APPOINTMENT

async def cancel_appointment_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отмена конкретной записи"""
    query = update.callback_query
    await query.answer()
    
    appointment_id = int(query.data.replace("cancel_", ""))
    appointment = get_appointment(appointment_id)
    
    if not appointment:
        await query.edit_message_text(
            "⚠️ Запись не найдена.",
            parse_mode=ParseMode.MARKDOWN
        )
        return ConversationHandler.END
    
    # Отменяем запись
    success = cancel_appointment(appointment_id, cancelled_by="client")
    
    if success:
        # Уведомляем клиента
        await query.edit_message_text(
            f"✅ *Запись #{appointment_id} отменена.*\n\n"
            f"Мы будем рады видеть вас снова!",
            parse_mode=ParseMode.MARKDOWN
        )
        
        # Уведомляем администратора
        for admin_id in ADMIN_IDS:
            try:
                await context.bot.send_message(
                    chat_id=admin_id,
                    text=f"""
❌ *ОТМЕНА ЗАПИСИ* #{appointment_id}

*Клиент:* {appointment['name']}
*Телефон:* {appointment['phone']}
*Услуга:* {appointment['service']}
*Дата:* {appointment['date']}
*Время:* {appointment['time']}

*Отменено:* клиентом""",
                    parse_mode=ParseMode.MARKDOWN
                )
            except Exception as e:
                logger.error(f"Не удалось отправить уведомление админу {admin_id}: {e}")
    else:
        await query.edit_message_text(
            "⚠️ Не удалось отменить запись. Попробуйте позже.",
            parse_mode=ParseMode.MARKDOWN
        )
    
    return ConversationHandler.END

# ========== АДМИН-ФУНКЦИИ ==========
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Панель администратора"""
    query = update.callback_query
    await query.answer()
    
    user = update.effective_user
    if user.id not in ADMIN_IDS:
        await query.edit_message_text("⛔ Доступ запрещен.")
        return ConversationHandler.END
    
    # Загружаем все записи
    data = load_appointments()
    appointments = data["appointments"]
    
    # Статистика
    total = len(appointments)
    active = sum(1 for a in appointments.values() if a.get("status") == "active")
    cancelled = sum(1 for a in appointments.values() if a.get("status") == "cancelled")
    
    keyboard = [
        [InlineKeyboardButton("📊 Статистика", callback_data="admin_stats")],
        [InlineKeyboardButton("📋 Все записи", callback_data="admin_all_appointments")],
        [InlineKeyboardButton("📅 Записи на сегодня", callback_data="admin_today")],
        [InlineKeyboardButton("📞 Контакты клиентов", callback_data="admin_contacts")],
        [InlineKeyboardButton("🔙 Главное меню", callback_data="back_to_menu")]
    ]
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(
        f"👑 *АДМИН-ПАНЕЛЬ*\n\n"
        f"*Всего записей:* {total}\n"
        f"*Активных:* {active}\n"
        f"*Отмененных:* {cancelled}",
        reply_markup=reply_markup,
        parse_mode=ParseMode.MARKDOWN
    )
    
    return ConversationHandler.END

async def admin_all_appointments(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Все записи для администратора"""
    query = update.callback_query
    await query.answer()
    
    user = update.effective_user
    if user.id not in ADMIN_IDS:
        return
    
    data = load_appointments()
    appointments = data["appointments"]
    
    if not appointments:
        await query.edit_message_text("📭 Нет записей.")
        return
    
    text = "📋 *ВСЕ ЗАПИСИ:*\n\n"
    
    for app_id, appointment in sorted(appointments.items(), key=lambda x: x[1].get("date", "")):
        status = appointment.get("status", "active")
        status_icon = "✅" if status == "active" else "❌" if status == "cancelled" else "📝"
        
        date_obj = datetime.strptime(appointment["date"], "%Y-%m-%d")
        formatted_date = date_obj.strftime("%d.%m.%Y")
        
        text += f"""
{status_icon} *Запись #{app_id}*
{appointment['service']}
*Клиент:* {appointment['name']}
*Телефон:* {appointment['phone']}
*Дата:* {formatted_date} {appointment['time']}
*Статус:* {status}
─────────────────────
"""
    
    keyboard = [
        [InlineKeyboardButton("🔄 Обновить", callback_data="admin_all_appointments")],
        [InlineKeyboardButton("🔙 В админ-панель", callback_data="admin_panel")]
    ]
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(
        text[:4000],  # Ограничение Telegram
        reply_markup=reply_markup,
        parse_mode=ParseMode.MARKDOWN
    )

async def admin_cancel_appointment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отмена записи администратором"""
    # Здесь можно добавить логику для отмены записей админом
    # Аналогично отмене клиентом, но с уведомлением клиенту
    pass

# ========== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ==========
async def back_to_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Возврат в главное меню"""
    query = update.callback_query
    await query.answer()
    
    return await start(update, context)

async def back_to_services(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Возврат к выбору услуги"""
    query = update.callback_query
    await query.answer()
    
    return await new_appointment(update, context)

async def contacts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Контакты салона"""
    query = update.callback_query
    await query.answer()
    
    await query.edit_message_text(
        """
📞 *КОНТАКТЫ САЛОНА*

*Адрес:* ул. Красоты, 15
*Телефон:* +7 (999) 123-45-67
*Email:* info@beautyelegance.ru
*Instagram:* @beautyelegance

*Часы работы:*
Пн-Пт: 9:00 - 21:00
Сб-Вс: 10:00 - 20:00

*Как добраться:* метро "Красивая", 5 минут пешком""",
        parse_mode=ParseMode.MARKDOWN
    )
    
    return ConversationHandler.END

async def about(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """О салоне"""
    query = update.callback_query
    await query.answer()
    
    await query.edit_message_text(
        """
🏩 *BEAUTY ELEGANCE*

*Премиальный салон красоты в центре города*

Мы работаем с 2015 года и предлагаем:
• Профессиональные услуги
• Качественную косметику
• Опытных мастеров
• Уютную атмосферу

*Наши преимущества:*
✅ Бесплатная консультация
✅ Гарантия на все услуги
✅ Подарочные сертификаты
✅ Система лояльности

*Наши мастера* — дипломированные специалисты с опытом работы от 5 лет.""",
        parse_mode=ParseMode.MARKDOWN
    )
    
    return ConversationHandler.END

async def cancel_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отмена диалога"""
    await update.message.reply_text(
        "Запись отменена. Чтобы начать заново, нажмите /start",
        reply_markup=ReplyKeyboardMarkup([[]], resize_keyboard=True)
    )
    
    context.user_data.clear()
    return ConversationHandler.END

# ========== ГЛАВНАЯ ФУНКЦИЯ ==========
def main():
    """Запуск бота"""
    # Создаем Application
    application = Application.builder().token(TOKEN).build()
    
    # Создаем ConversationHandler для записи
    conv_handler = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(new_appointment, pattern="^new_appointment$")
        ],
        states={
            SELECT_SERVICE: [
                CallbackQueryHandler(select_service, pattern="^service_"),
                CallbackQueryHandler(back_to_menu, pattern="^back_to_menu$")
            ],
            SELECT_DATE: [
                CallbackQueryHandler(select_date, pattern="^date_"),
                CallbackQueryHandler(back_to_services, pattern="^back_to_services$")
            ],
            SELECT_TIME: [
                CallbackQueryHandler(select_time, pattern="^time_"),
                CallbackQueryHandler(back_to_services, pattern="^back_to_services$")
            ],
            ENTER_NAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, enter_name)
            ],
            ENTER_PHONE: [
                MessageHandler(filters.CONTACT | filters.TEXT, enter_phone)
            ],
            ENTER_COMMENT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, enter_comment),
                CommandHandler("skip", enter_comment)
            ],
            CONFIRMATION: [
                CallbackQueryHandler(confirm_appointment, pattern="^confirm_appointment$"),
                CallbackQueryHandler(new_appointment, pattern="^edit_appointment$"),
                CallbackQueryHandler(cancel_conversation, pattern="^cancel$")
            ],
            CANCEL_APPOINTMENT: [
                CallbackQueryHandler(cancel_appointment_action, pattern="^cancel_"),
                CallbackQueryHandler(back_to_menu, pattern="^back_to_menu$")
            ]
        },
        fallbacks=[
            CommandHandler("cancel", cancel_conversation),
            CommandHandler("start", start)
        ],
        allow_reentry=True
    )
    
    # Регистрируем обработчики
    application.add_handler(CommandHandler("start", start))
    application.add_handler(conv_handler)
    
    # Обработчики callback-запросов
    application.add_handler(CallbackQueryHandler(my_appointments, pattern="^my_appointments$"))
    application.add_handler(CallbackQueryHandler(cancel_appointment_menu, pattern="^cancel_appointment$"))
    application.add_handler(CallbackQueryHandler(cancel_appointment_action, pattern="^cancel_appointment_list$"))
    application.add_handler(CallbackQueryHandler(contacts, pattern="^contacts$"))
    application.add_handler(CallbackQueryHandler(about, pattern="^about$"))
    application.add_handler(CallbackQueryHandler(admin_panel, pattern="^admin_panel$"))
    application.add_handler(CallbackQueryHandler(admin_all_appointments, pattern="^admin_all_appointments$"))
    application.add_handler(CallbackQueryHandler(back_to_menu, pattern="^back_to_menu$"))
    application.add_handler(CallbackQueryHandler(back_to_services, pattern="^back_to_services$"))
    
    # Запускаем бота
    print("🤖 Бот запущен! Нажмите Ctrl+C для остановки.")
    application.run_polling(allowed_updates=Update.ALL_TYPES)

if __name__ == "__main__":
    main()